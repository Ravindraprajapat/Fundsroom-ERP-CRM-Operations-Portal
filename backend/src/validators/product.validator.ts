export function validateCreateProduct(body: Record<string, unknown>): Record<string, string> {
  ...
  if (!body["productName"] || typeof body["productName"] !== "string" || !(body["productName"] as string).trim())
    errors["productName"] = "Product name is required";

  if (!body["sku"] || typeof body["sku"] !== "string" || !(body["sku"] as string).trim())
  ...
  const minStock = Number(body["minimumStock"] ?? 0);
...
export function validateUpdateProduct(body: Record<string, unknown>): Record<string, string> {
  ...
  if (body["currentStock"] !== undefined) {
    ...
    if (isNaN(stock) || stock < 0) errors["currentStock"] = "Stock must be non-negative integer";
  }

  if (body["minimumStock"] !== undefined) {
    const min = Number(body["minimumStock"]);
    if (isNaN(min) || min < 0) errors["minimumStock"] = "Minimum stock must be non-negative integer";
  }
